...
These are canonical role names. The actual label strings used in the issue tracker may differ. The mapping should have been provided to you. If not, tell the user to run `/me:setup`.
...
4. **Grill (if needed).** If the request needs fleshing out, call the Skill tool twice, for "me:brainstorming" and "me:domain-modeling", and grill it into shape a round of questions at a time, sharpening domain terms and updating `CONTEXT.md`/ADRs inline as decisions land.
