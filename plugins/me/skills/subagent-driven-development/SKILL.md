...
- Fresh subagent per task instead of one context doing every task
- Review after each task instead of only at the end
...
digraph process {
    ...
    "Use me:finishing-a-development-branch" [shape=box style=filled fillcolor=lightgreen];
    ...
    "Final review clean: delete this plan's workspace" -> "Use me:finishing-a-development-branch";
}
```
...
`me:using-git-worktrees` to create one or verify the existing one.
...
- Each plan owns a workspace: at skill start, run this skill's
  ...
  directory under `<repo-root>/.bstack/sdd/`, home to
...
- Check for this plan's ledger at `<workspace>/progress.md`. If its first
  ...
  ledger at the old flat path `.bstack/sdd/progress.md` — is another
...
- **Reviewer inputs:** the task reviewer gets three paths — the same brief
  ...
  constraints that bind the task. Include the plan's Review Focus items that
  apply to this task in `[REVIEW_FOCUS]`; use `None` when the plan has no such
  items.
...
The reviewer may also list behavior under "Declined to judge." Rule on every
line against the spec and reasonable user expectations, and record the
decision in the ledger with its cost if wrong. No declined behavior is silently
dropped.
...
`me:requesting-code-review`'s
[code-reviewer.md](../requesting-code-review/code-reviewer.md). Point it at
the plan's Review Focus section verbatim, if present, and the ledger's
deferred-minor and parked lines so it can triage which must be fixed before
merge.

Rule on every item in the final reviewer's "Declined to judge" list against
the spec and reasonable user expectations. Record each decision in the ledger
...
Use `me:finishing-a-development-branch`.
...
Done! Using `me:finishing-a-development-branch`.
```
