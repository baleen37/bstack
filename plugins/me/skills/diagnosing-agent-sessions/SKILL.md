---
name: diagnosing-agent-sessions
...
Pin down with your human partner what went wrong in an agent session, read the
transcripts on disk, and report what happened with evidence. Report the
involvement of `me` or other tools without claiming a plugin root cause;
whoever triages the bundle or issue decides whether `me` changes.
...
2. **Locate.** Resolve each session to verified absolute filesystem paths using
   ...
   `~/.bstack/diagnosing-agent-sessions/<session-id>/`, tell your
...
5. **GitHub issues** — when report §7 says possible or likely, or your
   partner asks. Resolve the destination repository using
   `references/github-issues.md`; never infer it from the project being
   diagnosed. Search open and closed issues for the symptoms. Show matches
   and suggest adding the report to the closest. If none match, fill
   `templates/issue.md`, write it to the workspace, show the exact text, and
   create the issue only after approval. `gh` cannot attach files; if a bundle
   exists, give your partner its path to attach in the browser.
...
- **Human prompts only.** Hook output, system reminders, and tool results
  ...
  parent agent.
- **No plugin root-cause claims.** Report §7 states whether `me` was
  involved and the evidence, then stops. Never name a defect in a skill or
  propose a change. Your partner pressing for a fix does not waive this;
  point at the issue step and mention that a bundle is available on request.
  No advice to your partner either.
