...
2. Using the skill-invocation and attribution meanings established in the case
   ...
   happened in.
3. List every plugin, skill, agent type, MCP server, or hook used that is not
   provided by `me`. Use only the evidenced tool, attribution, agent-dispatch,
   MCP, and hook meanings recorded in the case file; identify the source of
   each item.
4. For each human turn, compare the request text against the trigger
   ...
   - each plugin/skill/tool not provided by `me`, with where.
