...
`events`가 비어 있거나 `summary.clusters`와 `summary.signal_positions`가 모두 비었으면: "이 세션에서는 개선 신호를 못 찾았어요" 출력 후 종료.
...
2. Phase 0에서 받은 인덱스 JSON 전체 (`summary` + `events`만 있음). **먼저 `summary`를 읽어라** — `headline`(한 줄 상태), `clusters`(같은 kind 인접 turn ≥3회 묶음, `t_range`/`n`/`example_t` 포함), `signal_positions`(kind별 turn 좌표). 이걸로 어느 구간을 깊이 볼지 정한 뒤 `events[]`를 슬라이스해 인과 사슬 분석. summary는 단순 휴리스틱이라 false positive 가능 — 무의미한 cluster는 그냥 넘겨라.
...
4. 후보 파일 매핑 표 (분류 결과 + interrupt/repeat/error 이벤트 함께 보고):
   ...
   **"가까운" 해석**: "해당 SKILL.md"는 `events[]`의 `kind:"skill"` 항목이나 `prior`의 도구 호출에서 추정된 스킬의 SKILL.md. "가까운 AGENTS.md/CLAUDE.md"는 그 SKILL.md 디렉터리에서 상위로 올라가며 처음 만나는 AGENTS.md/CLAUDE.md (없으면 repo root). 추정 불가하면 repo root CLAUDE.md를 기본값으로.
