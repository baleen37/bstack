...
2. Phase 0에서 받은 인덱스 JSON 전체. 메인 데이터는 `events[]` (시계열). 보조: `skill_runs` (같은 스킬 N회 호출 한눈에), `signal_counts` (요약).
...
3. 사용자 응답에 따라:
   - **y**: patch를 임시 파일에 쓰고 `apply-patch.sh` 호출:
     ...
     > 스크립트가 외부 캐시 차단·dirty tree 차단·`git apply --check`·commit 메시지(`Signal:`/`Session:`)를 강제한다. 메인 에이전트가 직접 `git apply && git commit`하면 가드를 빠뜨릴 수 있어 분리.
