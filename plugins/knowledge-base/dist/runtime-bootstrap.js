...
import { copyFileSync, existsSync, linkSync, mkdirSync, readFileSync, renameSync, rmSync, unlinkSync, writeFileSync, } from "node:fs";
...
const npmArguments = ["ci", "--omit=dev", "--legacy-peer-deps", "--no-audit", "--no-fund"];
export function hasRuntimeDependencies(pluginRoot) {
    try {
        const marker = JSON.parse(readFileSync(resolve(pluginRoot, RUNTIME_STATE_DIRECTORY, READY_MARKER), "utf8"));
        return marker.lockfileSha256 === lockfileFingerprint(pluginRoot)
            && RUNTIME_DEPENDENCIES.every((dependency) => existsSync(resolve(pluginRoot, "node_modules", dependency, "package.json")));
    }
    catch {
        return false;
    }
...
function installRuntimeDependencies(pluginRoot, runner) {
    ...
    try {
        ...
        const result = runner(process.platform === "win32" ? "npm.cmd" : "npm", [...npmArguments], {
            cwd: stagingRoot,
            encoding: "utf8",
...
function hasStagedRuntimeDependencies(stagingRoot) {
    return RUNTIME_DEPENDENCIES.every((dependency) => existsSync(resolve(stagingRoot, "node_modules", dependency, "package.json")));
}
function promoteNodeModules(pluginRoot, stateRoot, stagingRoot) {
    ...
    try {
        ...
        if (hadPreviousInstall && existsSync(backup))
            renameSync(backup, target);
        throw error;
    }
    rmSync(backup, { recursive: true, force: true });
}
function writeReadyMarker(pluginRoot, stateRoot) {
    const markerPath = resolve(stateRoot, READY_MARKER);
...
function lockfileFingerprint(pluginRoot) {
    return createHash("sha256")
        .update(readFileSync(resolve(pluginRoot, "package-lock.json")))
        .digest("hex");
}
function isolatedBootstrapEnvironment(stateRoot) {
...
function withBootstrapLock(pluginRoot, operation) {
    const stateRoot = resolve(pluginRoot, RUNTIME_STATE_DIRECTORY);
