...
export async function openIndexStore(checkoutPath, paths) {
...
export async function openSearchStore(paths) {
    await prepareStore(paths);
    return createStore({ dbPath: paths.indexFile });
}
export async function indexKnowledge(store, scope, force, modelPath) {
...
export async function searchKnowledge(store, query, scope, limit) {
    if (query.trim() === "") {
    ...
    return results.map((result) => {
        ...
        const snippet = extractSnippet(result.body, query, 500, result.bestChunkPos, result.bestChunk.length);
        return {
...
export async function getKnowledge(store, ref, fromLine, maxLines) {
...
export async function getKnowledgeStatus(store) {
    const [status, health] = await Promise.all([store.getStatus(), store.getIndexHealth()]);
    return { status, health };
}
async function prepareStore(paths) {
...
async function verifyEmbeddingModel(modelPath) {
