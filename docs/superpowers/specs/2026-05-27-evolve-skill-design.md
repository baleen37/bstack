...
- 진화 단계의 자동화 수준은 **L2(제안 + 승인 후 적용)**. 자동 commit/push 안 함. 제안 전체를 plan으로 한 번에 제시하고 일괄 승인받되, **commit은 여전히 패치당 개별**이라 개별 `git revert`로 롤백 가능 (C안의 batch 롤백 위험 없음).
...
- **회고 보고서는 안 만든다**. 신호와 제안을 콘솔에 즉시 출력하고 plan으로 일괄 처리. 부산물 최소화.
...
interface Proposal {
  id: string;                // "P1", "P2", ...
  event_indexes: number[];   // 근거 신호 (events[] 인덱스)
  target_file: string;       // 절대 경로
  is_external_cache: boolean;
  change_kind: "edit";
  patch: string;             // unified diff 형식, 그대로 적용 가능
  rationale: string;         // 1~2문장
  addresses_signal: string;  // 자문 한 줄: event_indexes의 관찰된 실패 ↔ 패치가 그걸 막는 이유 (게이트 아님)
}
...
2. **전체 plan을 한 번에 제시**하고 단일 확인을 받음 (건별 프롬프트 금지). 각 항목은 full diff를 보여줌. 다중 세션은 `### <target_file>`로 그룹핑.
   ...
   P1  plugins/me/skills/browse/SKILL.md  —  <rationale>
       addresses: <addresses_signal>
       [patch diff]
   P2  ...

   적용? [all / none / <적용할 id, 예: P1 P3>]
   ```
   - `addresses_signal`은 자문 — 자동 제외 게이트 아님, 모든 비-external 제안을 노출.
   - patch를 고치려면 id를 제외하고 직접 편집 (inline edit 없음).
3. 선택된 id를 순서대로 `git apply` → 개별 commit (1 패치 = 1 commit, 메시지 형식 아래).
4. `git apply` 실패 시 **중단**: 이미 commit된 패치는 롤백 안 함(각자 독립). 실패 id·에러·이미 만든 SHA를 보고하고 사용자에게 넘김.
5. 적용할 게 없으면(전부 분리/제외) 그 사실을 알리고 마무리로.
6. 마무리: 적용된 commit sha 목록 + upstream 파일 경로 출력
